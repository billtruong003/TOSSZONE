...
## J. Bug mới owner báo từ playtest thật (2026-07-07 — TẤT CẢ ĐÃ VERIFY REPRO QUA MCP, CHƯA fix)

| ID | Hiện tượng | Nguyên nhân + BẰNG CHỨNG repro (MCP live) | Hướng fix đề xuất | Pri |
|---|---|---|---|---|
| PT-01 | Ring buff spawn quá thấp, sát/cắm đất | **Đã đo live CẢ HAI nơi.** Arena (slot rings): center y=1.46-2.16m. Hub training (`RingSpawnerHub` prefab `_zoneCenter`=NULL → fallback anchor y=2.3): center y=1.84-2.79m, sàn y=0. Về SỐ LIỆU tâm ring không "sát đất" — nhưng tier thấp (T1 đường kính 1.8m) tâm 1.46m → **mép dưới chỉ ~0.56m** — trong VR nhìn đúng là lết đất. Không phải bug logic đơn thuần mà là zone Y range quá thấp so với đường kính ring lớn | Nâng floor của công thức Y theo bán kính ring: `y = max(zoneY, floorY + ringRadius + clearance)` — tier to spawn cao hơn tương ứng; hoặc đơn giản nâng `zoneCenter.y` + kẹp mép dưới ≥1m | 🟡 |
| PT-02 | Bảng điểm giữa sân 2 mặt đè lên nhau, không đọc được (cả 2 phía) | **CONFIRMED live:** `ScoreboardUI` 2 mặt TMP cách nhau đúng 2cm, CÙNG text, shader `TextMeshPro/Mobile/Distance Field` với **`_CullMode=0` (double-sided)** — người đứng 1 phía thấy chữ mặt gần + chữ mặt xa hiện XUYÊN QUA dạng gương ngược, chồng lệch 2cm → không đọc được. Không có panel chắn giữa | **Fix rẻ nhất (1 dòng/mặt):** set `_CullMode=2` (Cull Back) trên fontMaterial cả 2 mặt — mỗi phía chỉ còn thấy đúng mặt của mình. Nâng cấp sau (theo ý owner): 1 bảng per-local-viewer billboard | 🔴 |
| PT-03 | Ring "kích hoạt" dù đạn không thực sự chạm/xuyên qua vòng | **CONFIRMED — NẶNG HƠN BÁO CÁO, 2 nguyên nhân:** ① **Đạn ĐƠN**: mesh `ColliderRing` (96 verts) có bounds gốc **211×210×4.9m** (author sai scale ~100×) + `MeshCollider convex=true` (hull torus = ĐĨA ĐẶC không lỗ) → mỗi ring là trigger đĩa ~150m trùm map, dày ~3.5m theo trục vòng. **Repro:** đạn bay cắt mặt phẳng Z cụm ring, cách mọi tâm >10m theo X → **3 ring bị tiêu thụ cùng lúc (5→2)**. Đây cũng chính là gotcha cũ "ring nuốt đạn test" (S13) chưa từng tìm ra nguyên nhân. ② **Đạn MƯA**: `TryStackThroughRing` chỉ so khoảng cách 0.4m tới TÂM — **repro:** 1 pellet, spread=0, bay thẳng cách tâm 0.35m dọc PHÁP TUYẾN (không bao giờ cắt mặt vòng) → ring vẫn consumed | ① Thay `ColliderRing` mesh bằng compound collider đúng scale (4-8 BoxCollider quanh vành, hoặc SphereCollider mỏng + plane-crossing check trong code) — hull convex torus KHÔNG BAO GIỜ phân biệt được lỗ. ② Burst check đổi sang plane-crossing + bán kính trong | 🔴 (nâng từ 🟡) |
| PT-04 | Đạn mưa (burst) bay xuyên qua người, không ngưng khi trúng | **CONFIRMED bằng chứng trực tiếp (reflection):** `Burst{Count=400}`: `SetDeadBit(10)→IsDead(10)=True` nhưng `SetDeadBit(300)→IsDead(300)=False` — `DeadMaskBits=256` < `MaxProjectilesPerBurst=4096`, `SetDeadBit` no-op index ≥256 trong khi hit-test scan hết `b.Count` → pellet ≥256 trúng damage nhưng không chết được: render bay xuyên + **damage lặp mỗi tick** | Tăng `DeadMaskBits` khớp 4096 (Dead0..63 = 64×ulong — cân nhắc size networked struct) HOẶC clamp `Count` thực tế ≤256 + tăng size viên để giữ mật độ hình ảnh | 🔴 |
| PT-05 | Đạn mưa (burst) hiện hình bóng tròn generic thay vì Đá | **CONFIRMED live:** `ProjectileBurstRenderer` đang chạy với `mesh=Sphere` (primitive fallback) + `material=BurstProjectile(runtime)` (cam, tạo runtime) — serialized field CHƯA BAO GIỜ được gán; hệ burst không nối `WeaponVisuals`/`WeaponConfig` | Ngắn hạn: gán mesh/material Đá vào 2 field serialized của `ProjectileBurstRenderer`. Dài hạn: thêm visual index vào `Burst` struct | 🟡 |
| PT-06 | Ném Đá — nổ/despawn giữa không trung ngang tầm người, CHƯA chạm đất | **CONFIRMED repro 100%:** đạn Đá (`SetAoe(0.8)` → `AreaScale=2.667`, hit-radius hiệu lực **0.8m** — đo live) bay ngang cách dummy 0.6m (chưa hề chạm) → **despawn giữa không trung tại (0.72, 1.20, 2.60), dummy mất máu 7→6**. `HitFirstVictim()` OverlapSphere 0.8m chạy MỌI tick từ lúc rời tay, không arm-gate (khác nhánh explosive có `IsArmed()`/0.7m) | ① Thêm arm-gate cho nhánh non-explosive; ② CÂN NHẮC giảm hit-radius Đá — 0.8m là "chạm" từ xa 0.7m tính từ mép bóng (bóng ~0.1m), cảm giác VR rất sai; damage-radius ≠ contact-radius | 🔴 |

**Setup repro chung (MCP):** tắt bot dummy, `RingSpawner._slotCount=0` + despawn ring cũ, spawn `DummyAvatar` làm mục tiêu (nhớ `HealCheat()` sau spawn — spawn ra Health=0), projectile spawn qua `Runner.Spawn(NetworkProjectile.prefab)` + `SetAoe(0.8)` + **`Shooter = LocalPlayer`** (để None sẽ trùng InputAuthority của dummy → bị skip như "chính chủ" — mất 1 lượt debug vì cái này).
