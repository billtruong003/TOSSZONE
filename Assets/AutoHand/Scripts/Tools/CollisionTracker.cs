    ...
    public class CollisionTracker : MonoBehaviour {
        ...
        private void CheckTrackedObjects() {
            if(!disableCollisionTracking) {
                for(int i = 0; i < collisionObjects.Count; i++) {
                    // TossZone patch (T14): a tracked object can be Destroy()'d (not just deactivated) between
                    // ticks — e.g. a networked grabbable despawned by a Fusion scene load while a DontDestroyOnLoad
                    // hand still references it. The symmetric nextCollisionObjects loop below already null-checks;
                    // this one didn't, throwing MissingReferenceException on .activeInHierarchy every tick after.
                    if(collisionObjects[i] == null) continue;
            ...
            if(!disableTriggersTracking) {
                for(int i = 0; i < triggerObjects.Count; i++) {
                    // TossZone patch (T14): same guard as the collision loop above.
                    if(triggerObjects[i] == null) continue;
